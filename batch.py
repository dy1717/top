...
for i in RunFiles:
    ...
    Dirname = "{}/src/nano/analysis/topMass/Batch/{}/".format(os.environ['CMSSW_BASE'],jobName)
    DirnameJDS = "{}/src/nano/analysis/topMass/Batch/".format(os.environ['CMSSW_BASE'])
    ...
    #Dirname_ = "/xrootd/store/user/dayoung/topmass_nano_180208/%s/"%(datasetName)
    if not os.path.isdir(Dirname_):
        os.makedirs(Dirname_)

    Dirname_ = "%s/src/nano/analysis/topMass/Batch/NanoAOD/"%(os.environ['CMSSW_BASE'])
    if not os.path.isdir(Dirname_):
        os.makedirs(Dirname_)
    ...
    for section in range(nSection):
        ...
        print>>fout, """executable = {0}/src/nano/analysis/topMass/Batch/ttbar.sh
universe   = vanilla
